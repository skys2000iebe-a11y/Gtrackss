document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderChart(range) {
        ...
        // Generate mock progress but incorporate real today's data
        const todayEntries = entries[todayStr] || [];
        const todayGoals = todayEntries.filter(e => e.type === 'goal');
        ...
        if (range === 'day') {
            ...
            dataPoints = [currentProg*0.8, currentProg*0.9, currentProg*0.5, currentProg*0.7, currentProg*0.6, currentProg*0.8, currentProg].map(p => Math.min(100, Math.max(0, p)));
        } else if (range === 'month') {
            labels = ['Week 1', 'Week 2', 'Week 3', 'This Week'];
            dataPoints = [currentProg*0.7, currentProg*0.8, currentProg*0.9, currentProg].map(p => Math.min(100, Math.max(0, p)));
        }
